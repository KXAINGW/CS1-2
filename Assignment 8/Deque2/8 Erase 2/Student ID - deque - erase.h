...
public:
   ...
   iterator erase( const_iterator where )
   {
      ...
      else
      {
         ...
         {
             // iterator
             iterator it_end = iterator(myData.myOff, &myData);
             iterator it = iterator(myData.myOff + off, &myData);
             for (; it != it_end; --it)
                 *it = *(it - 1);
             myData.myOff += 1;

             //cout << myData.myOff << ' ';
             //myData.myOff %= myData.mapSize * compDequeSize();   // error?!
         ...
         {
             iterator it = iterator(myData.myOff + off, &myData);
             iterator it_end = iterator(myData.myOff + myData.mySize, &myData);
             for (; it != it_end - 1; ++it)
                 *it = *(it + 1);
