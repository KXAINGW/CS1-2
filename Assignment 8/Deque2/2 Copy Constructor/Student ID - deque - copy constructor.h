...
public:
      ...
      : myData()
   {
      if( right.myData.mySize > 0 )
      {
         ...
         size_type block_end = getBlock(myData.myOff + myData.mySize - 1) + 1;
         //size_type block_num = right.getBlock(myData.myOff + myData.mySize - 1) - right.getBlock(myData.myOff) + 1;  // maybe cause error
         for (size_type i = 0; i < block_end; ++i)
         {
             myData.map[i] = new value_type[dequeSize];
             for (size_type j = 0; j < dequeSize && right_it != right.end(); ++j, ++right_it, ++it)
                 *it = *right_it;
         }
         */

         // 2 dimentional array #1
         /*
         size_type right_block_num = (right.myData.myOff + right.myData.mySize - 1) / dequeSize - right.myData.myOff / dequeSize + 1;
         size_type count = 0;
         for (size_type i = 0, i2 = right.myData.myOff; i < right_block_num; ++i, i2 += dequeSize)
         {
             myData.map[i] = new value_type[dequeSize];
             for (size_type j = 0; j < dequeSize && count < myData.mySize; ++j, ++count)
                 myData.map[i][j] = right.myData.map[right.getBlock(i2 + j)][(i2 + j) % dequeSize];
         }
         */

         // 2 dimensional array #2
         size_type count = 0;
         for (size_type i = 0, i2 = right.myData.myOff; i < right.myData.mapSize; ++i, i2 += dequeSize)
         {
             if (!right.myData.map[right.getBlock(i2)])
                 break;
             myData.map[i] = new value_type[dequeSize];
             for (size_type j = 0; j < dequeSize && count < myData.mySize; ++j,++count)
                 myData.map[i][j] = right.myData.map[right.getBlock(i2 + j)][(i2 + j) % dequeSize];
         ...
         // 2 dimensional array #3
         /*
         size_type count = 0;
         //cout << right.myData.myOff % dequeSize << ' ' << right.myData.myOff + right.myData.mySize << ' ' << (right.myData.myOff + right.myData.mySize) % dequeSize << endl;
         for (size_type i = 0, i2 = right.myData.myOff; i2 < (right.myData.myOff + right.myData.mySize); ++i, i2 += dequeSize)   // maybe cause error
         {
             myData.map[i] = new value_type[dequeSize]();
             for (size_type j = 0; j < dequeSize && count != myData.mySize; ++j, ++count)
                 myData.map[i][j] = right.myData.map[right.getBlock(i2 + j)][(i2 + j) % dequeSize];   // modify
         }
         */

         // simulate 1 dimensional array
         /*
         for (size_type i = 0, i2 = right.myData.myOff; i < right.myData.mySize; ++i)
         {
             if (!myData.map[i / dequeSize])
                 myData.map[i / dequeSize] = new value_type[dequeSize]();
             myData.map[i / dequeSize][i % dequeSize] = right.myData.map[((i2 + i) / dequeSize) % right.myData.mapSize][(i2 + i) % dequeSize];
         }
         */
