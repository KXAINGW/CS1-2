...
public:
      ...
      : myData()
   {
      ...
      else
      {
          ...
          while (count > (myData.mapSize - 1) * dequeSize)
              myData.mapSize *= 2;
          myData.myOff = 0;
          myData.map = new value_type * [myData.mapSize]();
          myData.mySize = 0;
          size_type block_end = getBlock(myData.myOff + count) + 1;
          //size_type block_end = (myData.myOff + count) / dequeSize % myData.mapSize + 1;

          for (size_type i = 0; i < block_end; ++i)
          {
              myData.map[i] = new value_type[dequeSize];
              for (size_type j = 0; j < dequeSize && myData.mySize != count; ++j, ++myData.mySize)
                  myData.map[i][j] = val;
          }
