      ...
      else
      {
         ...
         if( off <= myData.mySize / 2 )
         {
            if( myData.myOff % dequeSize == 0 &&
               ...
               doubleMapSize();
            size_type block_beg = getBlock(myData.myOff - 1);
            if (!myData.map[block_beg])
                myData.map[block_beg] = new value_type[dequeSize];
            iterator it = iterator(myData.myOff - 1, &myData);
            iterator it_end = iterator(myData.myOff + off , &myData);
            for (; it != it_end - 1; ++it)
                *it = *(it + 1);
            *it = val;
            myData.myOff -= 1;
            //cout << myData.myOff << "%" << myData.mapSize * dequeSize << "=" << myData.myOff % (dequeSize * myData.mapSize) << endl;
         ...
         else  // all elements after (and at) where move backward
         {
            if( ( myData.myOff + myData.mySize ) % dequeSize == 0 &&
               myData.mapSize <= ( myData.mySize + dequeSize ) / dequeSize )
               doubleMapSize();
            size_type block_end = getBlock(myData.myOff + myData.mySize);
            if (!myData.map[block_end])
                myData.map[block_end] = new value_type[dequeSize];
            iterator it = iterator(myData.myOff + myData.mySize, &myData);
            iterator it_beg = iterator((myData.myOff + off), &myData);
...
//      return ( ( myData.off / compDequeSize() ) & ( myData.mapSize - 1 ) );
   ...
   void doubleMapSize()
   {
      if( myData.mapSize > 0 )
      {
         ...
         for (size_type i = myData.myOff; myData.map[i / dequeSize % oldMapSize] && i < myData.myOff + myData.mySize; i += dequeSize)
             newMap[getBlock(i)] = myData.map[i / dequeSize % oldMapSize];

         //for (size_type i = getBlock(myData.myOff); i < getBlock(myData.myOff) + oldMapSize; ++i)
         //    if(myData.map[i%oldMapSize])
         //       newMap[i % myData.mapSize] = myData.map[i % oldMapSize];
         
         //size_type first = getBlock(myData.myOff);
         //for (size_type i = first; i < first + oldMapSize; ++i)
         //    if (myData.map[i % oldMapSize])
         //        newMap[i % myData.mapSize] = myData.map[i % oldMapSize];
