...
public:
      ...
      : myData()
   {
      if( right.myData.mySize > 0 )
      {
         ...
         myData.myOff = 0;
         size_type block_num = myData.mySize % dequeSize ? myData.mySize / dequeSize + 1 : myData.mySize / dequeSize;
         size_type right_block_pos = right.getBlock(right.myData.myOff);
         //size_type size = 0;
         //size_type right_j = right.myData.myOff % dequeSize;
         //for (size_type i = 0; i < block_num; ++i, ++right_block_pos)
         //{
         //    myData.map[i] = new value_type[dequeSize];
         //    if (right_block_pos == right.myData.mapSize)
         //        right_block_pos = 0;
         //    for (size_type j = 0; j < dequeSize && size != myData.mySize; ++j, ++right_j, ++size)
         //        myData.map[i][j] = right.myData.map[right_block_pos][right_j % dequeSize];
         //}
         ...
         const_iterator con_it = right.begin();
         size_type right_num = right.myData.mySize;
         for (size_type i = 0; i < block_num; ++i)
         {
             myData.map[i] = new value_type[dequeSize];
             for (size_type j = 0; j < dequeSize && con_it != right.end(); ++j, ++con_it)
                 myData.map[i][j] = *con_it;
         }
