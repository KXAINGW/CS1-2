...
public:
   ...
   deque& operator=( const deque &right )
   {
      if( &right != this ) // avoid self-assignment
      {
         ...
         size_type orign = myData.mapSize;
         ...
         {  // copy data from right to the current object
             ...
             //size_type block_beg = myData.myOff / dequeSize % myData.mapSize;
             //size_type block_end = (myData.myOff + right.myData.mySize - 1) / dequeSize % myData.mapSize + 1;
            
             //for (size_type i = block_beg; i != block_end; i++)
             //{
             //    //cout << i<<" "<<block_end<<" " << (i+1) % myData.mapSize << endl;
             //    if (block_end != myData.mapSize && i == myData.mapSize)
             //        i = 0;
             //    if (!myData.map[i])
             ...
             //iterator it = begin();
             //for (; right_it != right.end(); ++right_it, ++it)
             //    *it = *right_it;
...
//      return ( ( myData.off / compDequeSize() ) & ( myData.mapSize - 1 ) );
   ...
   void enlargeMap( size_type newMapSize )
   {
      ...
      if( myData.mySize > 0 )
      {
          size_type new_block_pos = getBlock(myData.myOff);
          size_type old_block_pos = new_block_pos;
          size_type dequeSize = compDequeSize();
          size_type block_num = oldMapSize % dequeSize ? oldMapSize / dequeSize + 1 : oldMapSize / dequeSize;
          for (size_type i = 0; i < block_num; ++i, ++new_block_pos, ++old_block_pos)
              newMap[new_block_pos] = myData.map[old_block_pos % oldMapSize];
         delete[] myData.map;
      }
